...
public class Node
{
	...
	public Node(float x, float y, Vector2 position, Pathfinding grid)
	{
		Initialize(x, y, position, grid);
	}

	public void Initialize(float x, float y, Vector2 position, Pathfinding grid)
	{
		X = (int)x;
		Y = (int)y;
	...
	public void CheckConnectionsPass1(Pathfinding grid)
	{
		if (!BadNode)
		{
	...
	public void InitializeConnections(Pathfinding grid)
	{
		bool valid = true;
		RaycastHit2D hit;
		float diagonalDistance = Mathf.Sqrt(Mathf.Pow(Pathfinding.UnitSize / 2f, 2) + Mathf.Pow(Pathfinding.UnitSize / 2f, 2));

		if (X > 1)
		{
			//Left
			valid = true;
			hit = Physics2D.Raycast(Position, new Vector2(-1, 0), Pathfinding.UnitSize);
		...
		{
			valid = true;
			hit = Physics2D.Raycast(Position, new Vector2(0, 1), Pathfinding.UnitSize);
			if (hit.collider != null && hit.collider.tag == "Wall")
			{
				valid = false;
			}
