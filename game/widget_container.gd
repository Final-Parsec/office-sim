...
func get_widget_at_position(position: Vector2) -> Node2D:
	for widget in get_children():
		var collision_shape = widget.get_node("CollisionShape2D") as CollisionShape2D
		var circle = collision_shape.shape as CircleShape2D
		var radius = circle.radius
		if position.distance_to(collision_shape.global_position) <= radius:
			return widget
			
	return null
	
func is_buildable_position(position: Vector2) -> bool:
